...
final class WeightTrackerViewModel {
    ...
    var nextCheckInLabel: String {
        ...
        let next = Calendar.current.date(byAdding: .day, value: 1, to: last) ?? Date()
    ...
    func logWeight() {
        ...
        if rounded < healthyMinLbs - 10 {
            ...
            motivationKind = .dangerHigh(bmi: b, targetLbs: r1(user.targetWeightLbs ?? healthyMaxLbs))
            fireConfetti   = false
            ...
            if improved {
                fireConfetti = true
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.15) { self.fireConfetti = false }
            } else {
                fireConfetti = false
            }
        }
...
private struct WeightTrackerContent: View {
    ...
    private var userCard: some View {
        HStack(spacing: 12) {
            ...
            VStack(spacing: 2) {
                ...
                Text("BMI").font(.system(size: 10)).foregroundColor(.secondary)
            }
        }
        .padding(14)
    ...
    private func stat(_ label: String, _ value: String, _ color: Color) -> some View {
        VStack(spacing: 4) {
            Text(label)
                ...
                .foregroundColor(.secondary).textCase(.uppercase).tracking(0.4)
            Text(value)
                .font(.system(size: 13, weight: .bold, design: .rounded))
                .foregroundColor(color).minimumScaleFactor(0.7).lineLimit(1)
        }
    ...
    private func legItem(_ c: Color, _ label: String, dashed: Bool) -> some View {
        HStack(spacing: 4) {
            if dashed {
                HStack(spacing: 2) {
                    ForEach(0..<3, id: \.self) { _ in Rectangle().fill(c).frame(width: 4, height: 2) }
                }.frame(width: 16)
            } else {
        ...
        return Chart {
            ...
            ForEach(vm.entries) { e in
                AreaMark(x: .value("Date", e.chartLabel), y: .value("Weight", e.weightLbs))
                    .foregroundStyle(fillColor).interpolationMethod(.catmullRom)
            }
            ForEach(vm.entries) { e in
            ...
            ForEach(vm.entries) { e in
                PointMark(x: .value("Date", e.chartLabel), y: .value("Weight", e.weightLbs))
                    .foregroundStyle(e.id == vm.entries.last?.id ? lineColor : lineColor.opacity(0.6))
            ...
            AxisMarks(values: .automatic(desiredCount: 5)) { v in
                AxisValueLabel {
                    if let n = v.as(Double.self) {
                        Text("\(Int(n)) lb").font(.system(size: 10)).foregroundStyle(Color.secondary)
                    }
                }
                AxisGridLine().foregroundStyle(Color(.systemGray5))
            }
    ...
    private var bmiSection: some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            HStack {
                Text("BMI zone").font(.system(size: 11)).foregroundColor(.secondary)
                Spacer()
                Text("\(String(format: "%.1f", b)) · \(vm.currentBMICategory)")
                    .font(.system(size: 12, weight: .bold)).foregroundColor(bmiColor(b))
            }
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    LinearGradient(
                        colors: [.blue, Color(red: 0.25, green: 0.72, blue: 0.55),
                                 Color(red: 0.25, green: 0.72, blue: 0.55), .orange, .red],
                        startPoint: .leading, endPoint: .trailing
                    )
                    .frame(height: 7).cornerRadius(4)
                    RoundedRectangle(cornerRadius: 2)
                        .fill(Color.primary).frame(width: 3, height: 15)
            ...
            HStack {
                ...
                Spacer()
                Text("Normal").font(.system(size: 9)).foregroundColor(Color(red: 0.25, green: 0.72, blue: 0.55))
                Spacer()
    ...
    private var inputSection: some View {
        HStack(spacing: 10) {
            TextField("Log today's weight (lbs)", text: $vm.inputText)
                ...
                .background(Color(.systemGray6))
                .cornerRadius(11)
                .overlay(RoundedRectangle(cornerRadius: 11).stroke(Color(.systemGray4), lineWidth: 0.5))
            Button(action: vm.logWeight) {
                Text("Log")
                    .font(.system(size: 14, weight: .bold, design: .rounded))
                    .foregroundColor(.white)
                    .padding(.horizontal, 22).padding(.vertical, 12)
                    .background(Color.orange).cornerRadius(11)
            }
        }
    ...
    private var historySection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Check-in history")
                ...
                .padding(.horizontal, 16)
            VStack(spacing: 5) {
                ForEach(vm.entries.reversed().prefix(8)) { entry in historyRow(entry) }
            }
            .padding(.horizontal, 16)
        }
        .padding(.top, 14)
    ...
    private func historyRow(_ entry: WeightEntry) -> some View {
        ...
        let diffColor: Color = {
            ...
            return (vm.isLoseGoal ? d < 0 : d > 0) ? Color(red: 0.25, green: 0.72, blue: 0.55) : .red
        }()
        return HStack {
            Text(entry.chartLabel).font(.system(size: 12)).foregroundColor(.secondary)
            Spacer()
...
private struct MotivationCard: View {
    ...
    private let loseMsgs: [(String, String, String)] = [
        ...
        ("figure.run", "That's real progress!", "Hard work is paying off. Keep showing up!"),
    ]
    ...
    private var c: MC {
        ...
        case .celebrateLose(let d):
            ...
            return MC(icon: m.0, title: m.1, body: m.2, badge: "▼ \(f1(d)) lb this check-in", accent: .red, bg: .red.opacity(0.07))
        case .celebrateGain(let d):
            let m = gainMsgs[Int.random(in: 0..<gainMsgs.count)]
            return MC(icon: m.0, title: m.1, body: m.2, badge: "▲ \(f1(d)) lb this check-in", accent: teal, bg: teal.opacity(0.08))
        case .noProgress(let d):
            return MC(icon: "chart.bar.fill", title: "Logged — stay consistent.",
                      body: "Weight \(isLoseGoal ? "went up" : "went down") by \(f1(d)) lbs. Progress isn't always linear — keep your habits strong.",
                      badge: "Keep showing up", accent: Color(.systemGray2), bg: Color(.systemGray6))
        case .dangerLow(let b, let minLbs):
            return MC(icon: "exclamationmark.triangle.fill", title: "Health concern — weight too low",
                      body: "At BMI \(f1(b)) you're below the safe minimum of \(f0(minLbs)) lbs. Please speak with your doctor.",
                      badge: "BMI \(f1(b)) · Consult a doctor", accent: .blue, bg: .blue.opacity(0.08))
        case .dangerHigh(let b, let targetLbs):
            return MC(icon: "exclamationmark.triangle.fill", title: "Health concern — high BMI range",
                      body: "BMI \(f1(b)) carries elevated risk. Your target of \(f0(targetLbs)) lbs would move you into a healthier zone.",
                      badge: "BMI \(f1(b)) · Consult a healthcare provider", accent: .red, bg: .red.opacity(0.07))
        case .passedHealthyMin(let b, let minLbs):
            return MC(icon: "hand.raised.fill", title: "Passed the healthy minimum",
                      body: "At BMI \(f1(b)) you've dropped below the healthy minimum of \(f0(minLbs)) lbs. Consider switching to maintenance.",
                      badge: "Consider switching to maintenance", accent: .orange, bg: .orange.opacity(0.08))
        }
    }
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 10) {
                ZStack {
                    ...
                    Image(systemName: c.icon).font(.system(size: 14, weight: .semibold)).foregroundColor(c.accent)
                }
                Text(c.title).font(.system(size: 14, weight: .bold))
            }
            Text(c.body).font(.system(size: 12)).foregroundColor(.secondary).lineSpacing(3)
            Text(c.badge)
                .font(.system(size: 11, weight: .bold)).foregroundColor(c.accent)
                .padding(.horizontal, 10).padding(.vertical, 4)
                .background(c.accent.opacity(0.12)).clipShape(Capsule())
        }
        .padding(14)
        .background(c.bg)
        .overlay(RoundedRectangle(cornerRadius: 12).stroke(c.accent.opacity(0.2), lineWidth: 0.5))
        .cornerRadius(12)
    }
}
...
struct ConfettiView: View {
    ...
    struct Piece: Identifiable {
        ...
        var rotation: Double; var size: CGFloat
        var color: Color; var isCircle: Bool
    }

    private let colors: [Color] = [
        Color(red: 0.86, green: 1.0, blue: 0.53), .orange,
        Color(red: 0.25, green: 0.72, blue: 0.55), .cyan, .red, .purple, .pink,
        Color(red: 1.0, green: 0.84, blue: 0.14)
    ]
    ...
    var body: some View {
        GeometryReader { _ in
            ZStack {
                ForEach(pieces) { p in
                    Group {
                        if p.isCircle { Circle().fill(p.color).frame(width: p.size, height: p.size) }
                        else { RoundedRectangle(cornerRadius: 2).fill(p.color).frame(width: p.size, height: p.size) }
                    }
    ...
    private func spawn() {
        ...
        pieces = (0..<75).map { _ in
            Piece(x: .random(in: 0...w), y: -20, rotation: .random(in: 0...360),
                  size: .random(in: 6...13), color: colors.randomElement()!, isCircle: Bool.random())
        }
